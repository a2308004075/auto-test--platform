...
export function createSyncConfig(projectId: number, data: { name: string; url: string; moduleId: number; headers?: string; hostPrefix?: string; authUsername?: string; authPassword?: string }) {
  return request.post(`/v1/projects/${projectId}/apis/sync-configs`, data)
}

export function updateSyncConfig(projectId: number, configId: number, data: { name: string; url: string; moduleId: number; headers?: string; hostPrefix?: string; authUsername?: string; authPassword?: string }) {
...
export function clearProjectApis(projectId: number) {
  return request.post(`/v1/projects/${projectId}/modules/clear-all-apis`)
}
