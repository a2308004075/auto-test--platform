-- =====================================================================
-- V26: 新增缺陷管理模块
-- 包含：缺陷分组、缺陷主表、工时、层级、关联、附件、变更记录、字典、菜单权�?
-- =====================================================================
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='缺陷分组�?;
...
CREATE TABLE `defect` (
  ...
  `assignee_id` bigint DEFAULT NULL COMMENT '负责�?ID',
  `due_date` date DEFAULT NULL COMMENT '计划完成时间',
  `found_version` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '发现的版�?,
  `module_name` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '所属模�?,
  ...
  `responsible_id` bigint DEFAULT NULL COMMENT '责任�?ID',
  `reopen_count` int NOT NULL DEFAULT '0' COMMENT '重新打开次数',
  `fixed_version` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '修改的版�?,
  `plan_test_date` date DEFAULT NULL COMMENT '计划提测时间',
  `status` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'NEW' COMMENT '状态：NEW-新建,PENDING-待验�?COMPLETED-已完�?REOPENED-重新打开,CLOSED-已关�?,
  `parent_id` bigint DEFAULT NULL COMMENT '父缺�?ID（层级关系）',
  `estimated_hours` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '总估算工�?,
  `actual_hours` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '总实际工�?,
  `remaining_hours` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '总剩余工�?,
  `created_by` bigint DEFAULT NULL COMMENT '创建�?ID',
  `updated_by` bigint DEFAULT NULL COMMENT '更新�?ID',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `defect_work_log` (
  ...
  `user_id` bigint DEFAULT NULL COMMENT '记录�?ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='缺陷工时记录�?;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='缺陷关联�?;
...
CREATE TABLE `defect_attachment` (
  ...
  `created_by` bigint DEFAULT NULL COMMENT '上传�?ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='缺陷附件�?;
...
CREATE TABLE `defect_history` (
  ...
  `old_value` text COLLATE utf8mb4_unicode_ci COMMENT '变更前�?,
  `new_value` text COLLATE utf8mb4_unicode_ci COMMENT '变更后�?,
  `changed_by` bigint DEFAULT NULL COMMENT '变更�?ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='缺陷变更记录�?;
...
('defect_status', '缺陷状�?, 'NEW', '新建', 1, '新建缺陷', 1, NOW(), NOW()),
('defect_status', '缺陷状�?, 'PENDING', '待验�?, 2, '待验�?, 1, NOW(), NOW()),
('defect_status', '缺陷状�?, 'COMPLETED', '已完�?, 3, '已完�?, 1, NOW(), NOW()),
('defect_status', '缺陷状�?, 'REOPENED', '重新打开', 4, '重新打开', 1, NOW(), NOW()),
('defect_status', '缺陷状�?, 'CLOSED', '已关�?, 5, '已关�?, 1, NOW(), NOW()),
('defect_severity', '严重级别', '致命', '致命', 1, '致命缺陷', 1, NOW(), NOW()),
('defect_severity', '严重级别', '严重', '严重', 2, '严重缺陷', 1, NOW(), NOW()),
('defect_severity', '严重级别', '一�?, '一�?, 3, '一般缺�?, 1, NOW(), NOW()),
('defect_severity', '严重级别', '提示', '提示', 4, '提示缺陷', 1, NOW(), NOW()),
('defect_source', '缺陷根源', '开发修改引�?, '开发修改引�?, 1, '开发修改引�?, 1, NOW(), NOW()),
('defect_source', '缺陷根源', '需求遗�?, '需求遗�?, 2, '需求遗�?, 1, NOW(), NOW()),
...
(112, 108, '状态流�?, 3, NULL, NULL, NULL, 'project:defect:status', 0, 1, NOW(), NOW()),
...
(121, '状态流�?, 'project:defect:status', 'BUTTON', 117, NULL, 4, 1, '缺陷状态流转按�?, 'display', NOW(), NOW()),
