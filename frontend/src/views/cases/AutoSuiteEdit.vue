<!--
 ...
 @description 自动化套件编辑视图（含三面板 Setup/Teardown 编排器）
-->
...
import { getAutoSuite, updateAutoSuite, getAutoSuiteLifecycle, saveAutoSuiteLifecycle } from '@/api/autoSuite'
...
const autoSuiteId = computed(() => Number(route.params.autoSuiteId))
...
const contextText = computed(() => '自动化套件级 · 整体 — 整个自动化套件执行前后各调用一次')
...
async function loadSuite() {
  loading.value = true
  try {
    const res: any = await getAutoSuite(projectId.value, autoSuiteId.value)
    const s = res.data
    Object.assign(form, {
      name: s.name || '',
  ...
  } catch { ElMessage.error('加载自动化套件失败') } finally { loading.value = false }
}
...
async function loadSuiteCases() {
  try {
    const res: any = await getAutoCases(projectId.value, { autoSuiteId: autoSuiteId.value, pageSize: 1000 })
...
async function loadLifecycle() {
  try {
    const res: any = await getAutoSuiteLifecycle(projectId.value, autoSuiteId.value)
    const list = (res.data || []) as any[]
    const map = new Map<number, any>()
    for (const item of list) { map.set(item.autoCaseId, item) }
    lifecycleItems.value = suiteCases.value.map(c => {
      const existing = map.get(c.id)
      return { autoCaseId: c.id, caseName: c.name, setupSteps: existing?.setupSteps || '', teardownSteps: existing?.teardownSteps || '' }
    })
  } catch { /* ignore */ }
}
...
async function handleSave() {
  ...
  saving.value = true
  try {
    await updateAutoSuite(projectId.value, autoSuiteId.value, {
      ...form,
    ...
    const items = lifecycleItems.value
      ...
      .map(item => ({ autoCaseId: item.autoCaseId, setupSteps: item.setupSteps || null, teardownSteps: item.teardownSteps || null }))
    await saveAutoSuiteLifecycle(projectId.value, autoSuiteId.value, { items })
  ...
  <div v-loading="loading">
    <div v-if="!loading">
      ...
      <EditPageHeader :title="pageTitle">
        <el-button v-if="hasPermission('project:auto-suite:edit')" type="primary" :loading="saving" @click="handleSave">保存</el-button>
      ...
      <el-card style="margin-bottom: 16px">
        <template #header>
          <div style="display: flex; align-items: center; justify-content: space-between">
            <span>自动化套件级 · 整体 Setup / Teardown</span>
        ...
        <div v-else style="color: #909399; text-align: center; padding: 20px">未启用自动化套件级整体生命周期</div>
      </el-card>

      <!-- 自动化套件内自动化用例级差异化生命周期 -->
      <el-card>
        <template #header>
          <span>自动化套件内自动化用例级差异化 Setup / Teardown</span>
        </template>
        <div style="color: #909399; font-size: 12px; margin-bottom: 8px">
          为自动化套件内每条自动化用例配置差异化的 Setup/Teardown 步骤。留空则使用自动化用例自身的配置。
        </div>
        <div v-if="lifecycleItems.length === 0" style="color: #909399; text-align: center; padding: 20px">
          自动化套件下暂无自动化用例
        </div>
        <el-collapse v-else accordion>
          <el-collapse-item v-for="item in lifecycleItems" :key="item.autoCaseId" :name="item.autoCaseId">
            <template #title>
            ...
            <el-row :gutter="16" style="margin-top: 8px">
              <el-col :span="12">
                ...
                <el-input v-model="item.setupSteps" type="textarea" :rows="6" placeholder="留空则使用自动化用例自身 Setup" style="margin-top: 6px; font-family: monospace; font-size: 12px" />
              </el-col>
              <el-col :span="12">
                <span style="font-weight: 600; font-size: 13px">差异化 Teardown</span>
                <el-input v-model="item.teardownSteps" type="textarea" :rows="6" placeholder="留空则使用自动化用例自身 Teardown" style="margin-top: 6px; font-family: monospace; font-size: 12px" />
              </el-col>
            </el-row>
          </el-collapse-item>
        </el-collapse>
