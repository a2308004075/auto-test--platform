...
import {
  getAutoSuites, createAutoSuite, updateAutoSuite, deleteAutoSuite,
  getAutoSuiteGroups, createAutoSuiteGroup, updateAutoSuiteGroup, deleteAutoSuiteGroup,
  getAutoSuitePassRates, batchUpdateAutoSuiteGroup, clearGroupAutoSuites, clearProjectAutoSuites,
} from '@/api/autoSuite'
...
async function fetchGroups() {
  try {
    const res: any = await getAutoSuiteGroups(projectId.value)
...
async function fetchList() {
  ...
  try {
    ...
    const res: any = await getAutoSuites(projectId.value, {
...
async function fetchPassRates() {
  ...
  try {
    ...
    const res: any = await getAutoSuitePassRates(projectId.value, ids)
    const map: Record<number, number> = {}
    for (const item of (res.data || [])) {
      map[item.autoSuiteId] = item.passRate
    }
...
async function batchDelete() {
  ...
  try {
    await ElMessageBox.confirm(`确定删除选中的 ${selectedIds.value.length} 个自动化套件？其下所有自动化用例将一并删除。`, '批量删除', { type: 'warning' })
    for (const id of selectedIds.value) {
      await deleteAutoSuite(projectId.value, id)
    }
...
async function handleBatchGroup() {
  if (selectedIds.value.length === 0) return
  try {
    await batchUpdateAutoSuiteGroup(projectId.value, { autoSuiteIds: selectedIds.value, groupId: batchGroupTarget.value })
...
async function handleSubmit() {
  if (!form.name) { ElMessage.warning('请输入自动化套件名称'); return }
  try {
    if (editingId.value) {
      await updateAutoSuite(projectId.value, editingId.value, { ...form })
      ElMessage.success('更新成功')
    } else {
      await createAutoSuite(projectId.value, { ...form })
...
function handleDelete(record: any) {
  ElMessageBox.confirm(`确定删除自动化套件「${record.name}」？其下所有自动化用例将一并删除。`, '确认删除', { type: 'warning' })
    .then(async () => { await deleteAutoSuite(projectId.value, record.id); ElMessage.success('删除成功'); fetchGroups(); fetchList() })
    .catch(() => {})
}
...
function contextClear() {
  ...
  ElMessageBox.confirm(
    isAll
      ? '确定清空项目下的所有自动化套件？其下所有自动化用例将一并删除，此操作不可恢复。'
      : isUngrouped
        ? '确定清空「未分组」中的所有自动化套件？其下所有自动化用例将一并删除，此操作不可恢复。'
        : `确定清空分组「${g.name}」及其子分组中的所有自动化套件？其下所有自动化用例将一并删除，此操作不可恢复。`,
    '确认清空',
    { type: 'warning', confirmButtonText: '清空', cancelButtonText: '取消' }
  )
    .then(async () => {
      if (isAll) {
        await clearProjectAutoSuites(projectId.value)
      } else {
        // 未分组（-1）后端语义为 groupId=0
        await clearGroupAutoSuites(projectId.value, isUngrouped ? 0 : g.id)
      }
...
async function handleGroupSubmit() {
  ...
  try {
    if (groupModalMode.value === 'create') {
      await createAutoSuiteGroup(projectId.value, { name: groupForm.name, description: groupForm.description, parentId: groupForm.parentId })
      ElMessage.success('分组创建成功')
    } else {
      await updateAutoSuiteGroup(projectId.value, editingGroupId.value, { name: groupForm.name, description: groupForm.description, parentId: groupForm.parentId })
...
async function handleDeleteGroup(group: any) {
  try {
    await ElMessageBox.confirm(`确定删除分组「${group.name}」吗？删除后，该分组下的自动化套件将自动归入「未分组」。`, '删除分组', { type: 'warning' })
    await deleteAutoSuiteGroup(projectId.value, group.id)
...
<template>
  <div>
    <PageHeader title="自动化套件">
      <el-button v-if="hasPermission('project:auto-suite:add')" type="primary" @click="openCreate">+ 新建自动化套件</el-button>
    </PageHeader>
      ...
      <div class="suite-content">
        <ProSearchCard :loading="loading" @search="handleSearch" @reset="handleReset">
          <div class="pro-search-field">
            <span class="pro-search-label">自动化套件名称</span>
            <el-input v-model="search.name" placeholder="搜索自动化套件名称" clearable style="width: 180px" @keyup.enter="handleSearch" />
          </div>
          <div class="pro-search-field">
        ...
        <el-table :data="list" v-loading="loading" border stripe style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="name" label="自动化套件名称" min-width="200">
            <template #default="{ row }">
              <div class="suite-name-cell">
                <span class="suite-name suite-name-link" @click="router.push(`/project/${projectId}/auto-suites/${row.id}/edit`)">{{ row.name }}</span>
                <div v-if="row.description" class="suite-desc">{{ row.description }}</div>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="caseCount" label="自动化用例数" width="100" align="center" />
          ...
          <el-table-column label="操作" width="210" fixed="right">
            <template #default="{ row }">
              <el-button type="primary" link size="small" @click="router.push(`/project/${projectId}/auto-cases?autoSuiteId=${row.id}`)">查看自动化用例</el-button>
              <el-button v-if="hasPermission('project:auto-suite:edit')" type="primary" link size="small" @click="router.push(`/project/${projectId}/auto-suites/${row.id}/edit`)">编辑</el-button>
              <el-button v-if="hasPermission('project:auto-suite:delete')" type="danger" link size="small" @click="handleDelete(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 新建/编辑自动化套件弹窗 -->
    <el-dialog v-model="modalVisible" :title="editingId ? '编辑自动化套件' : '新建自动化套件'" width="480px">
      <el-form label-position="top">
        <el-form-item label="自动化套件名称" required>
          <el-input v-model="form.name" placeholder="例如：冒烟自动化套件" />
        </el-form-item>
        <el-form-item label="描述">
          <el-input v-model="form.description" type="textarea" :rows="2" placeholder="自动化套件描述" />
        </el-form-item>
    ...
    <el-dialog v-model="batchGroupVisible" title="批量修改分组" width="420px">
      <p style="margin: 0 0 12px; color: #606266; font-size: 13px;">
        将选中的 <b style="color: #409eff">{{ selectedIds.length }}</b> 个自动化套件的分组修改为：
      </p>
      <el-tree-select
    ...
    <Teleport to="body">
      ...
      >
        ...
        <template v-if="!contextGroup">
          <div v-if="hasPermission('project:auto-suite:group')" class="context-menu-item" @click="contextCreateGroup">新建分组</div>
        </template>
        <!-- 系统分组（全部/未分组）右键：仅允许清空 -->
        <template v-else-if="contextGroup.isSystem === 1">
          <div class="context-menu-item danger" @click="contextClear">清空自动化套件</div>
        </template>
        <!-- 用户分组右键 -->
        <template v-else>
          <div v-if="hasPermission('project:auto-suite:group')" class="context-menu-item" @click="contextCreateChild">新建子分组</div>
          <div v-if="hasPermission('project:auto-suite:group')" class="context-menu-divider" />
